...
// const findNotes = (notes: Notes, note: string) =>
//   notes.find((el) => el.note === note);

// console.log(findNotes(upNotesPosition, 're'));

function Partition(props: Props) {
  const [spaceDevice, setSpaceDevice] = useState(0);
    ...
    <Svg width="100%" height="100%">
      ...
      <G id="layer1">
        <G id="g3451" transform="translate(0 55.714)">
          <Path
            id="path19"
            d="M22.36 125.874c2.308-1.692 4.031-2.973 5.141-3.858 1.11-.87 2.278-1.961 3.476-3.258A19.03 19.03 0 0034 114.362c.643-1.202 1.198-2.593 1.65-4.174.453-1.566.687-3.084.73-4.523 0-1.344-.16-2.625-.496-3.827-.321-1.217-.876-2.213-1.665-3.02-.789-.79-1.81-1.186-3.081-1.186-1.227 0-2.38.269-3.462.79-1.066.538-1.825 1.392-2.249 2.594 0 .11-.058.253-.146.459.03.253.146.442.365.585.22.142.41.205.585.205.087 0 .336-.047.715-.142.395-.095.716-.158.979-.158.774 0 1.46.3 2.088.885.613.585.92 1.297.92 2.135 0 .601-.16 1.17-.467 1.692a3.35 3.35 0 01-1.27 1.25c-.541.316-1.14.458-1.782.458-1.169 0-2.162-.38-2.98-1.154-.803-.791-1.212-1.787-1.212-3.037 0-1.597.453-2.972 1.344-4.143a8.308 8.308 0 013.446-2.624c1.388-.601 2.79-.886 4.22-.886 1.564 0 3.053.427 4.44 1.297 1.403.854 2.498 2.04 3.316 3.51a9.738 9.738 0 011.241 4.776c0 3.036-.935 5.85-2.804 8.46-1.87 2.609-4.177 4.87-6.937 6.8-1.84 1.312-4.82 3.083-8.923 5.312l-.204-.822zm20.286-24.463c0-.569.19-1.043.57-1.423a1.767 1.767 0 011.357-.585c.468 0 .906.222 1.3.648.394.412.584.902.584 1.44 0 .569-.204 1.059-.584 1.454-.409.38-.862.57-1.373.57-.54 0-.978-.19-1.329-.617-.35-.411-.525-.902-.525-1.487zm0 8.286c0-.569.19-1.06.54-1.439.365-.395.818-.585 1.387-.585.468 0 .891.206 1.3.633.38.427.584.885.584 1.391 0 .617-.19 1.107-.555 1.503-.38.395-.818.6-1.329.6a1.84 1.84 0 01-1.387-.585c-.35-.38-.54-.885-.54-1.518z"
            fill="#131516"
            stroke="#131516"
          ...
          {/* <Path
            ...
            // check theses fucking parameters
            // faire un array avec les coordonnées de chaques notes
            transform="rotate(180 184.641 184.161)"
            opacity={0.9}
            fill="#000"
            fillOpacity={1}
            fillRule="evenodd"
          ...
          {/* <Path
            ...
            // check theses fucking parameters
            // faire un array avec les coordonnées de chaques notes
            // transforme="rotate(180 x y)"
            transform="rotate(180 195.641 181.161)"
            opacity={0.9}
            fill="#000"
            fillOpacity={1}
            fillRule="evenodd"
          ...
          {/* <Path
            ...
            // check theses fucking parameters
            // faire un array avec les coordonnées de chaques notes
            transform="rotate(180 205.641 179.121)"
            opacity={0.9}
            fill="#000"
            fillOpacity={1}
            fillRule="evenodd"
          ...
          <Path
            d="M299.505 305.984v33.588"
            transform="rotate(180 205.641 177.161)"
            fill="none"
            fillOpacity={0.75}
            fillRule="evenodd"
            stroke="#000"
          ...
          {positions.map((el, i) => (
            <>
              <Path
                ...
                // faire un array avec les coordonnées de chaques notes
                // transforme="rotate(180 x y)"
                transform={`rotate(180 ${184 + i * 11} ${el})`}
                // transform="rotate(180 205.641 179.121)"
                opacity={0.9}
                fill="#000"
                fillOpacity={1}
